...
def index(request):
    ...
    context = RequestContext(request, {
        ...
        'latest_artists' : Artist.objects.filter(is_valid=True).order_by('-created')[:10],
        'latest_albums' : Album.objects.filter(is_valid=True).order_by('-created')[:10],
        'latest_links' : Link.objects.order_by('-created')[:10],
